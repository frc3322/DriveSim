...
public class ProfiledPIDAngleCommand extends CommandBase {
  TrapezoidProfile.Constraints m_constraints = new TrapezoidProfile.Constraints(3000, 4000);
  ...
  public void execute() {
    ...
    SmartDashboard.putNumber("AngleCommandTest/output", m_controller.calculate(robotDrive.getHeading(), goal));

    if(RobotBase.isSimulation()){
      robotDrive.arcadeDriveSim(0, m_controller.calculate(robotDrive.getHeading(), goal));
    } else {
      robotDrive.arcadeDrive(0, m_controller.calculate(robotDrive.getHeading(), goal));
    }
  }
